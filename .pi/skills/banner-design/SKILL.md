...
### Step 1: Gather Requirements (ask_user)
...
2. **Generate visual elements** with `ai-artist` + `ai-multimodal` skills
   ...
   > **Note:** `ai-artist` and `ai-multimodal` are optional skills and are NOT installed in this project.
   > If they are unavailable, skip AI image generation and build visuals with pure HTML/CSS
   > (gradients, geometric shapes, SVG patterns) instead.
