...
After generation, **ALWAYS** ask user about HTML preview via `ask_user`. If yes, invoke `/skill:ui-ux-pro-max` for gallery.
...
> **Note:** `frontend-design`, `ai-artist`, `ai-multimodal`, and `chrome-devtools` are optional skills not installed in this project. When referenced, fall back to: `ui-ux-pro-max` / `design-system` for design decisions, pure HTML/CSS for visuals, and any available headless-browser tool (e.g. `npx playwright` or Chrome headless CLI) for screenshot export.

Load `references/banner-sizes-and-styles.md` for complete sizes and styles reference.

### Banner: Workflow

1. **Gather requirements** via `ask_user` — purpose, platform, content, brand, style, quantity
...
1. **Orchestrate** — track TODO tasks; work through independent items in parallel
2. **Analyze** — Parse prompt: subject, platforms, style, brand context, content elements
3. **Ideate** — 3-5 concepts, present via `ask_user`
4. **Design** — `brand` skill → `design-system` skill → randomly invoke `/skill:ui-ux-pro-max` OR `the `design-system` skill`; HTML per idea × size
