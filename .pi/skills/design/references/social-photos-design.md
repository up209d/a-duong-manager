...
Work through independent tasks in parallel (e.g., multiple HTML files for different sizes).
...
Present ideas to user via `ask_user` for approval before designing.
...
3. **Randomly invoke ONE of:** `/skill:ui-ux-pro-max` OR `the `design-system` skill` — for layout, hierarchy, visual balance. Pick one at random each run for design variety.
